...
void UpdateTime(CBlockHeader* pblock, const CBlockIndex* pindexPrev)
{
	...
	int64_t nNow = GetAdjustedTime();
		...
		if((pindexPrev->GetBlockTime() + 1 + Params().TargetSpacing() + Params().TargetSpacing() - nNowBack) < GetAdjustedTime()){
			pblock->nTime = std::min(pindexPrev->GetBlockTime()+ 1 + Params().TargetSpacing() + 1800 , GetAdjustedTime());
			...
			pblock->nTime = std::min(pindexPrev->GetBlockTime()+ 1 + Params().TargetSpacing() + 25 , GetAdjustedTime());
